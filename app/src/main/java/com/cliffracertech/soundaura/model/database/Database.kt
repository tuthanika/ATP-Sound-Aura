    ...
    companion object {
        fun addAllMigrations(builder: Builder<SoundAuraDatabase>) =
            builder.addMigrations(migration1to2, migration2to3, migration3to4,
                                  migration4to5, migration5to6, migration6to7, migration7to8)
        ...
        private val migration7to8 = Migration(7, 8) { db ->
            db.execSQL("ALTER TABLE playlist ADD COLUMN `playSequentially` INTEGER NOT NULL DEFAULT 1")
            db.execSQL("ALTER TABLE track ADD COLUMN `loopEnabled` INTEGER NOT NULL DEFAULT 1")
        }
    }
}
